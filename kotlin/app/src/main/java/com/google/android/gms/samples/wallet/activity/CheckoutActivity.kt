...
import com.google.android.gms.samples.wallet.util.Notifications.triggerPaymentNotification
...
class CheckoutActivity : Activity() {
  ...
  private val shippingCost = (90 * 100).toLong()
  ...
  override fun onOptionsItemSelected(item: MenuItem): Boolean {
    return when (item.itemId) {
      R.id.send_notification -> {
        triggerPaymentNotification(this)
        true
      }
  ...
  private fun requestPayment() {
    ...
    val garmentPriceCents = (selectedGarment.getDouble("price") * 100).roundToLong()

    val paymentDataRequestJson = PaymentsUtil.getPaymentDataRequest(garmentPriceCents + shippingCost)
  ...
  private fun displayGarment(garment: JSONObject) {
    binding.detailTitle.text = garment.getString("title")
    binding.detailPrice.text = "\$${garment.getString("price")}"

    val escapedHtmlText:String = Html.fromHtml(garment.getString("description")).toString()
    binding.detailDescription.text = Html.fromHtml(escapedHtmlText)
